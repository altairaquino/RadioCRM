package com.grupooc.radiogrfm.struts.form;

import org.apache.struts.action.ActionForm;

public class FormContrato extends ActionForm {
	
	private static final long serialVersionUID = 1L;
	
	private String crncodg;
	private String crncgcl;
	private String crcnmcl;
	private String crcdccl;
	private String crcramo;
	private String crcnota;
	private String crncgmd;
	private String crcdcmd;
	private String crncgct;
	private String crcnmct;
	private String crncgag;
	private String crcnmag;
	private String crmtext;
	private String crdinic;
	private String crdterm;
	private String crnvalr;
	private String crnperm;
	private String cryperm;
	private String crlliqu = "T";
	private String crndesc;
	private String crncgfp;
	private String crcdcfp;
	private String crcpgto;
	private String crncgcp;
	private String crcdccp;
	private String crbcanc;
	private String crdcanc;
	private String crncoms;
	private String crycoms;
	private String crdcadt;
	private String crlativ;
	private String crncgep;
	private String crcnmep;	
	
	public String getCrlativ() {
		return crlativ;
	}
	public void setCrlativ(String crlativ) {
		this.crlativ = crlativ;
	}
	public String getCrncgep() {
		return crncgep;
	}
	public void setCrncgep(String crncgep) {
		this.crncgep = crncgep;
	}
	public String getCrcnmep() {
		return crcnmep;
	}
	public void setCrcnmep(String crcnmep) {
		this.crcnmep = crcnmep;
	}
	public String getCrcnota() {
		return crcnota;
	}
	public void setCrcnota(String crcnota) {
		this.crcnota = crcnota;
	}
	public String getCrdcadt() {
		return crdcadt;
	}
	public void setCrdcadt(String crdcadt) {
		this.crdcadt = crdcadt;
	}
	public String getCrncodg() {
		return crncodg;
	}
	public void setCrncodg(String crncodg) {
		this.crncodg = crncodg;
	}
	public String getCrncgcl() {
		return crncgcl;
	}
	public void setCrncgcl(String crncgcl) {
		this.crncgcl = crncgcl;
	}
	public String getCrcnmcl() {
		return crcnmcl;
	}
	public void setCrcnmcl(String crcnmcl) {
		this.crcnmcl = crcnmcl;
	}
	public String getCrcdccl() {
		return crcdccl;
	}
	public void setCrcdccl(String crcdccl) {
		this.crcdccl = crcdccl;
	}
	public String getCrncgag() {
		return crncgag;
	}
	public void setCrncgag(String crncgag) {
		this.crncgag = crncgag;
	}
	public String getCrcnmag() {
		return crcnmag;
	}
	public void setCrcnmag(String crcnmag) {
		this.crcnmag = crcnmag;
	}
	
	public String getCrmtext() {
		return crmtext;
	}
	public void setCrmtext(String crmtext) {
		this.crmtext = crmtext;
	}
	public String getCrdinic() {
		return crdinic;
	}
	public void setCrdinic(String crdinic) {
		this.crdinic = crdinic;
	}
	public String getCrdterm() {
		return crdterm;
	}
	public void setCrdterm(String crdterm) {
		this.crdterm = crdterm;
	}
	public String getCrnvalr() {
		return crnvalr;
	}
	public void setCrnvalr(String crnvalr) {
		this.crnvalr = crnvalr;
	}
	public String getCrndesc() {
		return crndesc;
	}
	public void setCrndesc(String crndesc) {
		this.crndesc = crndesc;
	}
	public String getCrncgfp() {
		return crncgfp;
	}
	public void setCrncgfp(String crncgfp) {
		this.crncgfp = crncgfp;
	}
	public String getCrcdcfp() {
		return crcdcfp;
	}
	public void setCrcdcfp(String crcdcfp) {
		this.crcdcfp = crcdcfp;
	}
	public String getCrncgcp() {
		return crncgcp;
	}
	public void setCrncgcp(String crncgcp) {
		this.crncgcp = crncgcp;
	}
	public String getCrcdccp() {
		return crcdccp;
	}
	public void setCrcdccp(String crcdccp) {
		this.crcdccp = crcdccp;
	}
	public String getCrbcanc() {
		return crbcanc;
	}
	public void setCrbcanc(String crbcanc) {
		this.crbcanc = crbcanc;
	}
	public String getCrdcanc() {
		return crdcanc;
	}
	public void setCrdcanc(String crdcanc) {
		this.crdcanc = crdcanc;
	}
	public String getCrncoms() {
		return crncoms;
	}
	public void setCrncoms(String crncoms) {
		this.crncoms = crncoms;
	}
	public String getCrlliqu() {
		return crlliqu;
	}
	public void setCrlliqu(String crlliqu) {
		this.crlliqu = crlliqu;
	}
	public String getCrnperm() {
		return crnperm;
	}
	public void setCrnperm(String crnperm) {
		this.crnperm = crnperm;
	}
	public String getCrcpgto() {
		return crcpgto;
	}
	public void setCrcpgto(String crcpgto) {
		this.crcpgto = crcpgto;
	}
	public String getCrncgmd() {
		return crncgmd;
	}
	public void setCrncgmd(String crncgmd) {
		this.crncgmd = crncgmd;
	}
	public String getCrcdcmd() {
		return crcdcmd;
	}
	public void setCrcdcmd(String crcdcmd) {
		this.crcdcmd = crcdcmd;
	}
	public String getCrncgct() {
		return crncgct;
	}
	public void setCrncgct(String crncgct) {
		this.crncgct = crncgct;
	}
	public String getCrcnmct() {
		return crcnmct;
	}
	public void setCrcnmct(String crcnmct) {
		this.crcnmct = crcnmct;
	}
	public String getCrcramo() {
		return crcramo;
	}
	public void setCrcramo(String crcramo) {
		this.crcramo = crcramo;
	}
	public String getCryperm() {
		return cryperm;
	}
	public void setCryperm(String cryperm) {
		this.cryperm = cryperm;
	}
	public String getCrycoms() {
		return crycoms;
	}
	public void setCrycoms(String crycoms) {
		this.crycoms = crycoms;
	}

}

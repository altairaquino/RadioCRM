package com.grupooc.radiogrfm.struts.form;

import org.apache.struts.action.ActionForm;

public class FormInsercao extends ActionForm {

	private static final long serialVersionUID = 1L;
	
	private String inncodg;
	private String inncgcr;
	private String innvlcr;
	private String inncgpg;
	private String incdcpg;
	private String innvlpg;
	private String inndrpg;
	private String inddata;
	private String inndesc;
	private String innvalr;
	private String inndura;
	private String innqtd;
	private String innsoma;
	
	public String getInnsoma() {
		return innsoma;
	}
	public void setInnsoma(String innsoma) {
		this.innsoma = innsoma;
	}
	
	public String getInncodg() {
		return inncodg;
	}
	public void setInncodg(String inncodg) {
		this.inncodg = inncodg;
	}
	public String getInncgcr() {
		return inncgcr;
	}
	public void setInncgcr(String inncgcr) {
		this.inncgcr = inncgcr;
	}
	public String getInnvlcr() {
		return innvlcr;
	}
	public void setInnvlcr(String innvlcr) {
		this.innvlcr = innvlcr;
	}
	public String getInncgpg() {
		return inncgpg;
	}
	public void setInncgpg(String inncgpg) {
		this.inncgpg = inncgpg;
	}
	public String getInnvlpg() {
		return innvlpg;
	}
	public void setInnvlpg(String innvlpg) {
		this.innvlpg = innvlpg;
	}
	public String getInndrpg() {
		return inndrpg;
	}
	public void setInndrpg(String inndrpg) {
		this.inndrpg = inndrpg;
	}
	public String getInddata() {
		return inddata;
	}
	public void setInddata(String inddata) {
		this.inddata = inddata;
	}
	public String getInnvalr() {
		return innvalr;
	}
	public void setInnvalr(String innvalr) {
		this.innvalr = innvalr;
	}
	public String getInndura() {
		return inndura;
	}
	public void setInndura(String inndura) {
		this.inndura = inndura;
	}
	public String getInnqtd() {
		return innqtd;
	}
	public void setInnqtd(String innqtd) {
		this.innqtd = innqtd;
	}
	public String getInndesc() {
		return inndesc;
	}
	public void setInndesc(String inndesc) {
		this.inndesc = inndesc;
	}
	public String getIncdcpg() {
		return incdcpg;
	}
	public void setIncdcpg(String incdcpg) {
		this.incdcpg = incdcpg;
	}
	
	
}
